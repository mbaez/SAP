...
# symbols which are imported by "from sap.command import *"
...
try:
    pkg_resources.require("TurboGears>=1.1.1")
...
from sap.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("sap"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
...
def bootstrap():
    ...
    By default this runs the function 'sap.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-sap = sap.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="Sistema-de-Administracion-de-Proyectos %s" % version)
    ...
    _read_config(args)
    from sap.model import bootstrap_model
...
def start():
    ...
    from sap.controllers import Root
